...
function normalizeLenses(lenses) {
  ...
  return list
    ...
    .map((lens) => ({
      name: typeof lens.name === "string" && lens.name ? lens.name : "Investor",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // "input" is an object matching our schema. // v7-normalize
    ...
    if (!parsed || typeof parsed !== "object") {
      if (stopReason === "max_tokens") {
        return res.status(502).json({
          error: "[v7] This report is exceptionally long and the critique exceeded the limit. Try a somewhat shorter report for now."
        });
      }
      return res.status(502).json({ error: "[v7] The AI did not return a structured result. Please try again." });
    }

    // The model usually returns "lenses" as a list, but occasionally as an
    // object keyed by lens name. Normalize either shape into a clean list so
    // the page never has to guess. Also guarantee praise/criticism are lists.
    const result = {
    ...
    if (!result.lenses.length) {
      return res.status(502).json({ error: "[v7] The AI returned no investor lenses. Please try again." });
    }
